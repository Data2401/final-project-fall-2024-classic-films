...
glimpse (raw_data)
```
Now, we also see that our data has a 
...
```
Given that distribution of films is ___ there are limitations ____.
...
raw_data |> group_by(Year)|> summarise(InflatedBudget = n()) |> ggplot(aes(Year,InflatedBudget)) +geom_point() + geom_smooth()
```
## Questions
