...
```
Notice the distribution of the films in the data. Out of 50 films, only 11 are pre-1990's while the remaining majority are from the early 1990's to the late 2010's. Keep in mind that this uneven distribution of films might 
impact the conclusions drawn from the data. 

Next, we wanted to find the average budget for each year. We wanted to see if the year had an impact on the budget. 
```{r}
...
Now we want to see, if the increase of budget means if there is an increase of gross. 
```{r}
...
which genre has the highest income? Which sex contributes more to that income. 
