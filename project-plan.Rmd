...
data <- data.frame(
  MovieName = c("Movie1", "Movie2", "Movie3"),
  Genre1 = c(0, 1, 1),
  Genre2 = c(1, 0, 0),
  Genre3 = c(1, 0, 1),
  MovieBudget = c(2500000, 3430000, 1240000)
)

# Finding the movie with the highest budget
result <- data %>%
  filter(MovieBudget == max(MovieBudget)) %>%
  select(MovieName, MovieBudget, contains("Genre")) %>%
  # Filtering only the genres that are 1 for the movie
  mutate(across(starts_with("Genre"), ~ifelse(. == 1, 1, NA_integer_))) %>%
  # Remove NA columns, keeping only genres with 1
  select_if(~!all(is.na(.)))
