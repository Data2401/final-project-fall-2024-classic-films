...
quantity_films
```
How many films of each genre are there? 
```{r}
...
The graph is not wrong, but rather both variables have a difference of ... Let's arrange these graphs separatly so we can make the comparison more accurate. 
```{r}
...
What films made the most and least grossings? What genres were these films?
...
```{r Arranging code for genre manipulation}
...
Now we'll plot each genre, and their grossings by sex.
...
Enjoyment is classified as... Below, we'll plot the rated enjoyment by genre. 
```{r}
...
Apreciation is classified as ... Below, we'll plot apreciation by genre. 
```{r}
...
We'll now compare enjoyment and appreciation by genre. 
```{r}
