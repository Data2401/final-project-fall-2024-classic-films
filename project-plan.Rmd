...
Now, we'll look at the year disturbution of our film entries. Does the data contain multiple films per year? Is so, what is the count for each year? 
```{r}
...
The graph is not wrong, but rather both variables have a difference of millions of dollars.Concerns about the visual data might being misdirect.To prevent that the graph was arrange separately so we can make the comparison more accurate. 
```{r}
