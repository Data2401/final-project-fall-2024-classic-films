...
```{r}
#avg_year_budget <- raw_data |> group_by( Year) |> summarize(average_yearly_budget = mean(InflatedBudget))
#avg_year_budget

# or we can see if there is a trend in budget by the years
...
#raw_data |> slice_max(InflatedBudget) |> select(FilmName, Year, InflatedBudget, InflatedGross) |> select ()

#raw_data |> filter(InflatedBudget == max(InflatedBudget)) |> select(FilmName, Year, InflatedBudget, InflatedGross, contains"m")

#Attempting to create a modified data frame for genreFAILED 
#LINK[https://www.geeksforgeeks.org/how-to-filter-r-dataframe-by-values-in-a-column/#]
#genreframe<- raw_data[raw_data$col24==1]
#genreframe
#pivot_longer(raw_data, -c(FilmName, InflatedBudget)) |> slice_max(InflatedBudget) |> filter(value == 1) |> pivot_wider()
```
...
```{r}
#created a new data for genre but not sure if it was correct way to combat the genre question

genre <- raw_data |> reframe(across(c(FilmName,Crime, Drama, Western, Action, Biography, History, Adventure,Fantasy, Mystery, SciFi, Romance, Thriller, War, Family, Animation,Comedy,InflatedBudget,InflatedGross)))


#genre |> group_by(c(Crime, Drama, Western, Action, Biography, History, Adventure,Fantasy, Mystery, SciFi, Romance, Thriller, War, Family, Animation,Comedy)) |> summarise(genre_count = n())
