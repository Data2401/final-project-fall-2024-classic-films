...
library(yaml) #Loaded to knit as custom .pptx template
library(devtools) #Loaded for patchwork
library(patchwork) #Loaded to organize plots 
library(reshape2) # Loaded to
...
## Data Analysis 
```{r echo=FALSE, fig.height=6, fig.width=7.51}
...
(meanBudget_Genre + meanGross_Genre)
...
```{r fig.height=6, fig.width=7.51}
combined_df <- merge(genre_app, genre_enj, by="genre", all=TRUE)
ggplot(combined_df,aes(x=genre)) + 
          geom_bar(aes(y=meanenj),stat="identity",position="dodge",col="blue") +
          geom_bar(position="dodge") +   
          geom_bar(aes(y=meanapp),stat="identity",position="dodge",col="red")
combined_df.long<-melt(combined_df)
ggplot(combined_df.long,aes(genre,value, fill=variable))+
     geom_bar(stat="identity",position="dodge")+facet_wrap(~genre)
