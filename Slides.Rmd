...
ggplot(count_movieby_year, aes(x = Year, y = Movie_Count)) + geom_col() + labs(x = 'Year', y = 'Film Count', title = 'Film Release Distribution by Year in Dataset') + theme(plot.title = element_text(hjust = 0.5))
```

```{r include=FALSE}
Avg_Budget_by_Year <- raw_data |> group_by(Year)|> summarise(InflatedBudget) |> ggplot(aes(Year,InflatedBudget)) +geom_point() + geom_smooth()+labs(x = 'Year', y = 'Inflated Budget', title = 'Average Film Budget by Year') + theme(plot.title = element_text(hjust = 0.5))
```
